...
exports.insertNewTicker = async (symbol, millisec) => {
  ...
  try {
    ...
    if (tableExist.rows.length === 0) {
      ...
      if(	parseInt(lastTimestamp.rows[0].max ,10)+ millisec <= 	new Date().getTime() ) {

        const {data}= await axios.get(`https://api.binance.com/api/v3/klines?symbol=${symbol.toUpperCase()}&interval=1m&startTime=${parseInt(lastTimestamp.rows[0].max ,10)+60000}&limit=1`); // get data from binance
        ...
        console.log('symbol ' + symbol + ' timestamp ' + (parseInt(lastTimestamp.rows[0].max ,10)+60000) + ' =  result ' + data[0][0]);
      }
      else {
